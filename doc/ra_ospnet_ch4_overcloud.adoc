...
connected to one of the two switches, creating redundancy should one
of the switches go down.
...
- Provisioning – VLAN 190
- Internal API – VLAN 1095
- Storage – VLAN 1097
- Storage Management – VLAN 1098
- Tenant (tunneling) – VLAN 1099
- External – VLAN 192
- Floating IP – VLAN 193
...
----
 ...
 parameter_defaults:
  ...
  InternalApiNetworkVlanID: 1095
  StorageNetworkVlanID: 1097
  StorageMgmtNetworkVlanID: 1098
  TenantNetworkVlanID: 1099
  ExternalNetworkVlanID: 192

  # Set to empty string to enable multiple external networks or VLANs
  NeutronExternalNetworkBridge: "''"
  # Customize bonding options
  BondInterfaceOvsOptions:
      "mode=4 lacp_rate=fast updelay=1000 miimon=100"
----
...
Each subnet has a range of IP addresses that is used for assigning IP
addresses to hosts and virtual IPs. Looking closely at the subnets
configured in the environment example above:
...
Other options are available to influence Linux kernel bonds:
...
upstream switch. Instead, you can use active-backup mode, or OVS can use
_balance-slb_  mode to enable using two interfaces with the same VLANs
configured as a bond, see below.

=== Configuring OVS Bonding Options

Additional options are available for OVS bonds.
...
OVS will traffic based on source MAC address and destination
 VLAN. A given VLAN will only use one link out of the bond, but
 multiple VLANs can be spread across the bond members.
 The switch only sees a given MAC address on a VLAN on one link in the
...
Here is the full set of service-to-net mappings, this can be included in the environment file, and then the networks
can be customized.
...
In order to use isolated networks, the servers must have IP addresses on each network.
The Undercloud uses a local Neutron server to manage IP addresses on the isolated networks,
so we need to enable Neutron port creation for each network. We override the resource
registry in our environment file.

First, this is the complete set of networks and ports that can be deployed (including the
...
resource_registry:
  ...
  OS::TripleO::Controller::Ports::ManagementPort:
    /usr/share/openstack-tripleo-heat-templates/network/ports/management.yaml
...
to use the third and fourth NICs for a Linux kernel bond:
...
When using numbered interfaces ("nic1", "nic2", etc.) instead of named interfaces ("eth0", "eno2", etc.), the network
interfaces of hosts within a role do not have to be exactly the same. For instance, one host may have interfaces em1
and em2, while another has eno1 and eno2, but both hosts' NICs can be referred to as nic1 and nic2.

The numbered NIC scheme only takes into account the interfaces that are live (have a cable attached to the switch). So
if you have some hosts with 4 interfaces, and some with 6, you should use nic1-nic4 and only plug in 4 cables on each
host.

The nic numbering counts any NIC starting with "eno*", or "em*" first
in alphanumeric order, since those are assumed to be embedded NICs.
The rest of the NICs are then counted in alphanumeric order.
...
For stability, OSP-Director will configure IP routes statically.
...
By default, {ro} and above uses static IP addressing on all interfaces.
...
{ro} configures Neutron with an empty string for the Neutron external
...
IP network. This reduces future flexibility, however, since an additional
interface cannot be added to a separate bridge.
...
NOTE: When moving the address (and possibly route) statements onto the bridge,
be sure to remove the corresponding VLAN interface from the bridge. Make sure
to make the changes to all applicable roles. The External network is only on
the controllers, so only the controller template needs to be changed. The
Storage network on the other hand is attached to all roles, so if the storage
network were on the native VLAN, all roles would need to be edited.
...
==== Changes to Network Configuration to Support Linux Kernel Bonding
The syntax for using Linux bonds and bridges is
...
Here is a comparison of the OVS LACP bonds used in {ro} and the
...
==== Deploying With Native IPv6 Networking
The isolated Overcloud networks may be deployed with
either IPv4 or IPv6 networks on a per-network basis. Due to PXE boot
support and IPMI support (for HA fencing, IPMI is used for STONITH, where
a node is deactivated by issuing power commands via IPMI) being limited to
IPv4, it is required that the Provisioning network be a routable IPv4
network when IPv6 isolated networks are used.

In the directories that contain the sample network interface configurations,
you will find an alternate version of the _controller.yaml_, named
_controller-v6.yaml_, which illustrates how to provide both an IPv6 default
gateway on the External (Public API) network, and an IPv4 default gateway
on the Provisioning network.

For example, here is a closer look at the template in
_network/config/bond-with-vlans/controller-v6.yaml_:
                ...
                -
                  ...
                  routes:
                    *-*
                      ...
                      *next_hop: {get_param: ExternalInterfaceDefaultRoute}*
----
Apart from the differences in routing for the Controllers, the main differences
in configuration for an IPv6 deployment have to do with setting the IP subnet
properties correctly in the _network-environment.yaml_. Let's look at a
version of the reference architecture _network-environment.yaml_ for use with
IPv6 Overcloud networks:
...
----
 resource_registry
  ...
  OS::TripleO::Compute::Net::SoftwareConfig:
    /home/stack/nic-configs/compute.yaml
  OS::TripleO::Controller::Net::SoftwareConfig:
    */home/stack/nic-configs/controller-v6.yaml*
  OS::TripleO::ObjectStorage::Net::SoftwareConfig:
    /home/stack/nic-configs/swift-storage.yaml
  OS::TripleO::CephStorage::Net::SoftwareConfig:
    /home/stack/nic-configs/ceph-storage.yaml
 ...
 parameter_defaults:
  ...
  # Leave room if the external network is also used for floating IPs
  ExternalAllocationPools: [{'start': '2001:db8::10', 'end': '2001:db8::100'}]
  # Gateway router for the external network
...
To deploy with IPv6 native addressing on all networks, use the network-isolation-v6.yaml
template:
