...
OVS will balance traffic based on source MAC address and destination
 VLAN. The switch will only see a given MAC address on one link in the
bond at a time, and OVS will use special filtering to prevent packet
...
By default, {ro} director 7.1 will use static IP addressing on all interfaces. To set a static route on an interface with a static IP, specify a route to the subnet. For instance, here is a hypothetical route to the 10.1.2.0/24 subnet via the gateway at 172.17.0.1 on the Internal API network:
...
{ro} 7 will configure Neutron with an empty string for the Neutron external bridge mapping. This results in the physical interface being patched to br-int,rather than using br-ex directly (as in previous versions). This model allows for multiple floating IP networks, using either VLANs or multiple physical connections.
...
The 7.1 version of {ro} director supports static IPs on the Provisioning network. These changes require additional parameters for setting static IPs, routes, and DNS servers. If you are reusing templates from an {rpo} 7.0 deployment, you will have to modify the templates to be compatible with version 7.1.
...
The NIC templates for each role will have to be modified as well. First, additional parameters need to be added to the top section of the NIC config templates. Whether the provisioning interface will use DHCP or static IPs, these need to be added in any case:
...
If the provisioning interface will use static IPs, then those parameters need to be used in the NIC config portion of the templates. Here is an example of the changes required for the controller role (in bold):
